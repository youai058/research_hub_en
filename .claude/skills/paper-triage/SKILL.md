...
python3 .claude/skills/paper-triage/scripts/collect_abstracts.py \
    --glob "papers/**/*.raw.md" > /tmp/triage_input.json
...
우선순위:
1. `--topic-from <slug>` 주어짐 → `python3 topic_log.py load <slug>`로 텍스트 로드 (`--topic`과 **mutually exclusive**)
2. `--topic "<string>"` 주어짐 → 그대로 사용
3. 둘 다 없음 → exit 2
...
```
papers/etc/2026/foo.raw.md
papers/etc/2026/bar.raw.md
```
