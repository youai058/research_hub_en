...
| paper-summarizer | adaptive Marp 요약 (PLANNING-first, 4 앵커), 비판적 독해 | A-3 | papers |
...
STAGE_SUBPHASES = {
    ...
    "qa":          ["B-1", "B-2"],
    "experiments": ["E-1", "E-2", "E-3"],  # C-1은 Phase A(experiment-design 스킬)가 담당; Phase C는 E-1부터 실행
    "analyze":     ["F-1", "F-2"],
}
```
...
1. **`papers/vector_db/kg.stale` 확인**: SessionStart 훅이 `kg_state` 변수를 제공하거나 직접 `ls papers/vector_db/kg.stale` 체크.
   - 존재하면 kg-curator 호출: `/research-kg build` 실행.
   - 성공 시 스크립트가 `.stale`을 자동 제거.
2. **`papers/vector_db/rejected.jsonl` 증분 읽기**:
   - orchestrator는 `papers/vector_db/.rejected_cursor` (단일 정수 줄 수)를 유지.
